...
public class NowPlaying : Singleton<NowPlaying>
{
    ...
    public void Initialized( Song _data )
    {
        ...
        {
            bpms.Add( new BPMS( timings[i].bpm, timings[i].time ) );
        }
    ...
    /// <param name="_time"></param> Bpm에 따라 이 시간이 줄어들거나 늘어납니다.
    /// <param name="_timings"></param> 한 곡에 대한 bpm 리스트
    /// <returns></returns> 변환 된 _time 값
    public static float GetChangedTime( float _time, Timing[] _timings ) // BPM 변화에 따른 시간 계산
    {
        double newTime = _time;
        double prevBpm = 0d;
        for ( int i = 0; i < _timings.Length; i++ )
        {
            double time = _timings[i].time;
            double bpm = _timings[i].bpm;
