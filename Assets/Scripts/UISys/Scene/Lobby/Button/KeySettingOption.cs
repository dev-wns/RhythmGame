...
public class KeySettingOption : OptionButton
{
    ...
    {
        currentKeyCode = GameSetting.Inst.Keys[( GameKeyAction )lane];
        while ( true )
        {
            yield return null;
            ...
            if ( Input.GetKeyDown( KeyCode.Return ) )
            {
                for ( int i = 0; i < 6; i++ )
                {
                    if ( currentKeyCode == GameSetting.Inst.Keys[( GameKeyAction )i] )
                    {
                ...
                GameSetting.Inst.Keys[( GameKeyAction )lane] = currentKeyCode;
